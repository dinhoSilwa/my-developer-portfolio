...
export const NavBar = () => {
  ...
  return (
    <nav className="w-[100vw] px-[10%] relative h-24 flex justify-end items-center gap-12">
      <div className="font-primary text-[12px] font-bold text-light-primary uppercase flex items-end gap-1 flex-1">
        Claudio silva
      </div>

      <div
        className={`w-screen h-screen lg:w-auto lg:h-auto lg:relative lg:top-0 lg:py-0 lg:flex-row lg:left-0 absolute top-20 left-1/2 py-24 transform transition-all duration-500 ease-in-out  bg-dark-primary/[95%] flex flex-col items-center gap-12 justify-start text-light-primary font-second ${
          isOpenMobileMenu
      ...
      <div
        className={`flex gap-4 transition-all duration-300 ${
          isOpenMobileMenu
            ? "absolute top-24"
            : "absolute top-24 transform translate-x-[120%] md:top-0 md:translate-x-0 md:translate-y-0 md:relative"
        } `}
      >
        {navIcons.map(({ name, nav, icon }, index) => (
